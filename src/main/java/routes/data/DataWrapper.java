...
public class DataWrapper {
    ...
    private static Connection getConnection() throws ClassNotFoundException, SQLException {
        ...
        System.out.println("Connection to DB was set");
        return conn;
    }

    private static void closeDB(Connection conn) throws  SQLException {
        conn.close();
    ...
    public static ArrayList<Place> getPlacesByCategoryId(long categoryId, double x, double y, double r) throws  ClassNotFoundException, SQLException {
        ...
        String query = String.format("SELECT Category_id, x, y, name FROM Places " +
                "WHERE Category_id = %d AND ((x - %f) * (x - %f) + (y - %f) * (y - %f) <= %f * %f)", categoryId, x, x, y, y, r, r);
